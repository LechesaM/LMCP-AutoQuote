...
RUNTIME_DIR = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "rfq_document_acquisition"
DOWNLOAD_DIR = RUNTIME_DIR / "downloads"
REPORT_DIR = RUNTIME_DIR / "reports"
LIVE_BUYER_PACK_DIR = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "live_buyer_packs"
...
def _ensure_dirs() -> None:
    ...
    LIVE_BUYER_PACK_DIR.mkdir(parents=True, exist_ok=True)
...
def _extract_seed_urls(item: Dict[str, Any]) -> List[str]:
    ...
    return [
        u
        for u in _unique_preserve_order(urls)
        if _is_http_url(u) and (_looks_like_downloadable_document(u) or _is_specific_tender_detail_url(u))
    ]


def _is_specific_tender_detail_url(url: str, link_text: str = "") -> bool:
    blob = f"{url} {link_text}".lower()
    if _looks_like_downloadable_document(url, link_text):
        return True
    if any(term in blob for term in ("tender", "tenders", "rfq", "rfp", "bid", "opportunity", "notice", "document")) and not any(
        term in blob for term in ("home", "index", "category", "archive", "listing", "portal", "news")
    ):
        return True
    return False


def _buyer_pack_path(buyer_name: str, reference: str) -> Path:
    buyer = _slug(buyer_name or "buyer")
    ref = _slug(reference or "rfq")
    return LIVE_BUYER_PACK_DIR / buyer / ref


def _write_live_buyer_pack(
    buyer_name: str,
    reference: str,
    title: str,
    result: Dict[str, Any],
    ...
    if not downloaded_files:
        return {"live_buyer_pack_path": "", "live_buyer_pack_manifest_path": ""}

    pack_dir = _buyer_pack_path(buyer_name, reference)
    ...
    for row in downloaded_files:
        if not isinstance(row, dict):
            continue
        path_value = _safe_str(row.get("path"))
        src = Path(path_value)
        ...
        try:
            ...
            continue
        copied.append({**row, "copied_path": str(dest)})
    ...
    manifest = {
        ...
        "live_buyer_pack_path": str(pack_dir),
        "downloaded_files": copied,
        "source_report_path": result.get("report_path", ""),
        "document_confidence_score": result.get("document_confidence_score", result.get("confidence", 0.0)),
    }
    manifest_path = pack_dir / "live_buyer_pack_manifest.json"
    try:
    ...
    return {
        "live_buyer_pack_path": str(pack_dir),
        "live_buyer_pack_manifest_path": str(manifest_path) if str(manifest_path) else "",
    }
...
def acquire_rfq_documents(item: Dict[str, Any], timeout_seconds: int = DEFAULT_TIMEOUT_SECONDS) -> Dict[str, Any]:
    ...
    pack_result = _write_live_buyer_pack(
        _safe_str(item.get("buyer_name")),
        _safe_str(item.get("buyer_rfq_number") or item.get("rfq_number") or item.get("reference_number") or title),
        title,
        result,
        successful_downloads,
    )
