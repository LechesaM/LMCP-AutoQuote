...
SUPPLIER_DATA_FILE = os.getenv(
    ...
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "suppliers_master.json")
)
...
DEFAULT_SUPPLIERS: List[SupplierRecord] = [
    ...
    SupplierRecord(
        ...
        products=[
            ...
            SupplierProduct("Printer toner", "stationery_office", 950.00, "each", 3, 1200),
    ...
    SupplierRecord(
        ...
        products=[
            ...
            SupplierProduct("Door", "building_materials", 1350.00, "each", 5, 3000),
        ],
    ),
    SupplierRecord(
        supplier_id="SUP-004",
...
def _save_default_suppliers() -> None:
    ...
    for supplier in DEFAULT_SUPPLIERS:
        payload.append(asdict(supplier))
...
def find_supplier_options(
    ...
    matches.sort(key=lambda x: (x["unit_price"], x["lead_time_days"], -x["available_stock"]))
    return matches
