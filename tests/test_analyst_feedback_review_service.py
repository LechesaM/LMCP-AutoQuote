...
def test_analyst_feedback_review_service_returns_supervised_review_snapshot(tmp_path) -> None:
    module = importlib.import_module("app.services.analyst_feedback_review_service")
    service = module.AnalystFeedbackReviewService(runtime_dir=tmp_path / "runtime" / "staging" / "recommendation-feedback-governance")

    latest = service.latest_analyst_feedback_review()
    history = service.analyst_feedback_review_history(limit=5)
