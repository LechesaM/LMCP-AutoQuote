...
def _write_cycle(
    ...
    generated_at: str,
    *,
    queue_status: str = "PASS",
    telemetry_status: str = "PASS",
    retry_status: str = "PASS",
    lock_status: str = "PASS",
    dry_run_status: str = "PASS",
    no_go_status: str = "PASS",
    readiness_score: float = 100.0,
    operator_frequency: float = 0.0,
) -> None:
...
def test_operational_remediation_service_tracks_overdue_blockers_and_deadlines(tmp_path) -> None:
    module = importlib.import_module("app.services.operational_remediation_service")
    cycle_root = tmp_path / "runtime" / "staging" / "pilot-cycles"
    export_root = tmp_path / "runtime" / "staging" / "governance-exports"
    _write_cycle(
        cycle_root,
        "cycle-open",
        "2026-06-20T11:30:52+00:00",
        queue_status="FAIL",
    ...
    (export_dir / "pilot_rehearsal_summary.json").write_text(json.dumps({"export_id": "export-1", "generated_at": "2026-06-20T11:09:09+00:00", "readiness_summary": {"readiness_score": 68.0, "readiness_grade": "watch"}}), encoding="utf-8")

    service = module.OperationalRemediationService(cycle_root=cycle_root, export_root=export_root)
    latest = service.latest_remediation()

    assert latest["status"] == "watch"
    assert latest["remediation_summary"]["open_remediation_count"] >= 1
    assert latest["remediation_summary"]["overdue_remediation_count"] >= 1
    assert latest["remediation_summary"]["blocking_remediation_count"] >= 1
    assert latest["operational_risk_closure_summary"]["overdue_count"] >= 1
    assert latest["open_remediation_tracking"]
    assert latest["unresolved_blocker_tracking"]
    assert any(item["accepted_operational_risk_classification"] in {"accepted", "not_accepted"} for item in latest["remediation_actions"])
    assert latest["warning_indicators"]["overdue_remediation_warning"] is True


def test_operational_remediation_service_tracks_completed_history(tmp_path) -> None:
    module = importlib.import_module("app.services.operational_remediation_service")
    cycle_root = tmp_path / "runtime" / "staging" / "pilot-cycles"
    export_root = tmp_path / "runtime" / "staging" / "governance-exports"
    _write_cycle(cycle_root, "cycle-1", "2026-06-01T11:30:52+00:00", queue_status="FAIL", telemetry_status="FAIL", retry_status="FAIL")
    _write_cycle(cycle_root, "cycle-2", "2026-06-23T11:30:52+00:00")
    export_dir = export_root / "export-1"
    export_dir.mkdir(parents=True, exist_ok=True)
    ...
    service = module.OperationalRemediationService(cycle_root=cycle_root, export_root=export_root)
    latest = service.latest_remediation()
    history = service.remediation_history(limit=5)

    assert latest["status"] == "ok"
    assert latest["remediation_summary"]["resolved_remediation_count"] >= 1
    assert latest["remediation_governance_history"]["remediation_count"] >= 1
    assert history["count"] >= 1
    assert history["resolved_remediation_history"]
    assert history["operational_risk_closure_summary"]["closed_count"] >= 1
