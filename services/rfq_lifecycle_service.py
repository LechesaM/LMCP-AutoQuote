...
TERMINAL_REVIEW_BLOCKERS = {
    "closing_date_passed",
    "not_supply_and_delivery",
    "missing_closing_date",
}
MANUAL_PRICING_DIR = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "manual_pricing"

DISCOVERY_STORE_CANDIDATES = [
    Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "live_rfqs.json",
    Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "multi_portal_discovery" / "qualified_candidates_report.json",
    Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "multi_portal_discovery" / "eligible_candidates_report.json",
    Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "opportunity_extraction" / "high_confidence_opportunities.json",
]
...
MANUAL_SUBMISSION_READY_STATES = {"SUBMISSION_READY_MANUAL", "SUBMISSION_READY"}
...
class RfqLifecycleService:
    ...
    def _is_lifecycle_qualified(self, payload: Dict[str, Any]) -> bool:
        ...
        if not accepted:
            return False
        document_ok, _ = self._document_quality_gate(payload)
        if not document_ok:
            return False
    ...
    def _normalize_item(self, payload: Dict[str, Any], source: str = "api_ingest") -> Dict[str, Any]:
        ...
        item = {
            ...
            "policy": {
                ...
                "minimum_document_confidence": MIN_DOCUMENT_CONFIDENCE,
    ...
    def _live_store_index(self) -> Dict[str, Dict[str, Any]]:
        ...
        live_store = RUNTIME_DIR / "live_rfqs.json"
    ...
    def _document_confidence_score(self, payload: Dict[str, Any]) -> float:
        scores = [
    ...
    def _document_quality_gate(self, payload: Dict[str, Any]) -> Tuple[bool, List[str]]:
        reasons: List[str] = []
        closing_ok, closing_reason = self._closing_date_status(payload)
        if not closing_ok:
            reasons.append(closing_reason)
        score = self._document_confidence_score(payload)
        if score and score < MIN_DOCUMENT_CONFIDENCE:
            reasons.append("document_confidence_below_threshold")
        if not score:
            reasons.append("missing_document_confidence")
        if not _http_urls_from_payload(payload):
            reasons.append("missing_downloadable_document_url")
        if not (payload.get("document_paths") or payload.get("document_verified") is True):
            reasons.append("missing_document_backing")
        if bool(payload.get("briefing_required")):
            reasons.append("briefing_required")
        reasons.extend(str(reason) for reason in (payload.get("blocker_reasons") or []) if str(reason).strip())
    ...
    def _closing_date_value(self, payload: Dict[str, Any]) -> str:
    ...
    def _closing_date_status(self, payload: Dict[str, Any]) -> Tuple[bool, str]:
        value = self._closing_date_value(payload)
        if not value:
    ...
    def _manual_pricing_totals(self, line_items: List[Dict[str, Any]], validation_issues: Optional[List[str]] = None) -> Dict[str, Any]:
        subtotal_ex_vat = round(sum(_safe_float(item.get("total_ex_vat") or item.get("total") or 0.0, 0.0) for item in line_items), 2)
        vat_total = round(sum(_safe_float(item.get("vat_amount") or item.get("vat") or 0.0, 0.0) for item in line_items), 2)
        grand_total_inc_vat = round(sum(_safe_float(item.get("total_incl_vat") or 0.0, 0.0) for item in line_items), 2)
        total_cost = round(sum(_safe_float(item.get("unit_cost_total") or 0.0, 0.0) for item in line_items), 2)
    ...
    def _manual_pricing_validation(self, payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any], List[str]]:
        ...
        blockers = list(totals.get("blockers") or [])
    ...
    def _download_artifact_groups(self, downloads: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        groups = {
            "documents": [],
            "pricing_schedules": [],
            "boqs": [],
        ...
        def _entry(row: Dict[str, Any], artifact_type: str) -> Dict[str, Any]:
            path = str(row.get("path") or "")
            url = str(row.get("url") or row.get("final_url") or "")
            name = Path(path).name if path else Path(url.split("?", 1)[0]).name
            return {
        ...
        for row in downloads:
            ...
            document = _entry(row, "Document")
            groups["documents"].append(document)

            if any(token in blob for token in ("pricing schedule", "price schedule", "schedule of prices", "quotation schedule")):
                groups["pricing_schedules"].append(_entry(row, "Pricing Schedule"))
            if any(token in blob for token in ("boq", "bill of quantities")):
                groups["boqs"].append(_entry(row, "BOQ"))
            if "sbd" in blob:
                groups["sbd_forms"].append(_entry(row, "SBD Form"))
            if any(token in blob for token in ("specification", "scope of work", "terms of reference", "technical")):
                groups["specifications"].append(_entry(row, "Technical Specification"))
            if any(token in blob for token in ("annex", "returnable", "declaration", "form", "spec", "rfq", "tender")):
                groups["buyer_forms"].append(_entry(row, "Buyer Form"))

        for key, values in groups.items():
            deduped: List[Dict[str, Any]] = []
            seen = set()
            for value in values:
                token = f"{value.get('type')}|{value.get('path') or value.get('url') or value.get('name')}".lower()
                if token in seen:
                    continue
                seen.add(token)
    ...
    def save_manual_pricing(self, rfq_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if totals.get("verified"):
            ...
            self._append_audit(
                item,
                "manual_pricing_saved",
                reason="manual pricing requires review",
                blockers=blockers,
                success=False,
            )
        ...
        try:
            ...
            LiveRFQStore.upsert({
                **item,
                "validation_status": "pricing_verified" if totals.get("verified") else "manual_pricing_required",
                "pipeline_status": "pricing_verified" if totals.get("verified") else "manual_pricing_required",
                "status": "Pricing Verified" if totals.get("verified") else "Manual Pricing Required",
                "quote_ready": bool(totals.get("verified")),
                "eligible": bool(totals.get("verified")),
                "blocked": not bool(totals.get("verified")),
                "manual_pricing_path": item.get("manual_pricing_path", ""),
                "manual_pricing_totals": totals,
                "manual_pricing_line_items": line_items,
            })
        except Exception:
            pass
    ...
    def _build_quote_pack_step(self, priced_payload: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            return generate_quote_pack_from_v43_payload(
                ...
                output_dir=str(RUNTIME_DIR / "rfq_lifecycle" / "quote_packs"),
            )
        except Exception as exc:
    ...
    def _generate_controlled_proof_package(self, item: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            proof_root = RUNTIME_DIR / "rfq_lifecycle" / "proofs" / f"{_slug(rfq_id)}-{stamp}"
    ...
    def run_controlled_submission(self, limit: int = 25) -> Dict[str, Any]:
        items = [item for item in self.store.list_items() if item.get("current_state") in MANUAL_SUBMISSION_READY_STATES]
    ...
    def _terminal_review_blocker_codes(self, item: Dict[str, Any], enriched: Dict[str, Any]) -> List[str]:
        ...
        reasons.update(self._split_reason_codes((item.get("review_recovery_decision") or {}).get("reason")))
                ...
                processed.append(
                    {
                        ...
                        "result": "skipped",
                        "blocker_reasons": self._split_reason_codes(item.get("failure_reason")),
                    }
                )
                continue

            item["terminal_review_cleanup"] = {
                "action": "rejected",
    ...
    def _write_scale_proof_manifest(self, rfq_id: str, payload: Dict[str, Any]) -> str:
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        proof_dir = RUNTIME_DIR / "rfq_lifecycle" / "scale_simulation" / stamp
    ...
    def run_live_acquisition_validation(self, limit: int = 25, timeout_seconds: int = 6) -> Dict[str, Any]:
        ...
        report_path = RUNTIME_DIR / "rfq_lifecycle" / "live_acquisition_validation.json"
    ...
    def _cleanup_directory(self, path: Path, cutoff_epoch: float, allow_delete_submitted_proofs: bool = False, dry_run: bool = False) -> Dict[str, Any]:
        ...
        if not path.exists():
            result["skipped"] = True
            return result
        real_proofs_dir = RUNTIME_DIR / "rfq_lifecycle" / "proofs"
    ...
    def cleanup_runtime(self, retention_days: int = 14, allow_delete_submitted_proofs: bool = False, dry_run: bool = False) -> Dict[str, Any]:
        ...
        targets = [
            RUNTIME_DIR / "rfq_lifecycle" / "scale_simulation",
            RUNTIME_DIR / "rfq_lifecycle" / "tmp",
            RUNTIME_DIR / "rfq_lifecycle" / "temp",
            RUNTIME_DIR / "rfq_document_acquisition" / "reports",
        ]
        ...
        if allow_delete_submitted_proofs:
            cleanup_rows.append(self._cleanup_directory(RUNTIME_DIR / "rfq_lifecycle" / "proofs", cutoff_epoch, allow_delete_submitted_proofs=True, dry_run=dry_run))
    ...
    def run_live_pilot(
        ...
        for item in pilot_items:
            ...
            try:
                ...
                self._transition(item, "SUBMISSION_READY_MANUAL", "live_pilot_quote_pack_ready_no_submission", event="live_pilot")
    ...
    def advance_parsed(self, limit: int = 25) -> Dict[str, Any]:
        ...
        for item in items:
            ...
            try:
                ...
                self._transition(item, "SUBMISSION_READY_MANUAL", "quote_pack_ready_no_submission_executed", event="advance_parsed")
                advanced_count += 1
    ...
    def validate_visible_opportunities(
        ...
        live_data = LiveRFQStore.get_all()
        live_items = [item for item in (live_data.get("items") if isinstance(live_data, dict) else []) or [] if isinstance(item, dict)]
        visible_items = live_items[: max(1, int(limit))]
        ...
        for source_item in visible_items:
            ...
            detail_url = str(enriched.get("detail_url") or "")
            document_url = str(enriched.get("document_url") or "")
            closing_ok, closing_value = self._closing_date_status(enriched)
            document_confidence_score = self._document_confidence_score(enriched)
            existing_paths = [str(path) for path in item.get("document_paths", []) if path]
            ...
            quote_pack: Dict[str, Any] = {}
            local_pack_result: Dict[str, Any] = {}

            if not str(enriched.get("buyer_name") or item.get("buyer_name") or "").strip():
                blocker_reasons.append("missing_buyer_name")
            if not str(enriched.get("source") or item.get("source") or "").strip():
                blocker_reasons.append("missing_source_name")
            if not (document_url or detail_url or source_url or urls):
            ...
            if document_confidence_score < MIN_DOCUMENT_CONFIDENCE:
                blocker_reasons.append("document_confidence_below_threshold")
            if not document_confidence_score:
                blocker_reasons.append("missing_document_confidence")

            if not blocker_reasons or not set(blocker_reasons).issuperset({"missing_source_or_detail_url"}):
                acquisition = self._document_acquisition_step(
                    enriched,
                    timeout_seconds=timeout_seconds,
                    max_concurrent_downloads=max_concurrent_downloads,
            ...
            downloaded_paths = [
                str(row.get("path"))
                for row in downloads
                if isinstance(row, dict) and str(row.get("status") or "") == "downloaded" and row.get("path")
            ]
            combined_paths = sorted(set(existing_paths + downloaded_paths))
            if combined_paths:
                item["document_paths"] = combined_paths
                item["document_acquisition_report_path"] = acquisition.get("report_path", item.get("document_acquisition_report_path", ""))
                item["document_acquisition_confidence"] = acquisition.get("confidence", item.get("document_acquisition_confidence", 0.0))
                item["document_confidence_score"] = acquisition.get("document_confidence_score", acquisition.get("confidence", document_confidence_score))
                item["live_buyer_pack_path"] = acquisition.get("live_buyer_pack_path", item.get("live_buyer_pack_path", ""))
                item["live_buyer_pack_manifest_path"] = acquisition.get("live_buyer_pack_manifest_path", item.get("live_buyer_pack_manifest_path", ""))
            else:
                blocker_reasons.append("missing_documents")

            parse_payload = dict(enriched)
            parse_payload["document_paths"] = combined_paths
            parse_payload["document_url"] = urls
            if combined_paths:
                parsing = self._document_parse_step(parse_payload, timeout_seconds=timeout_seconds)
                intelligence = parsing.get("document_intelligence") if isinstance(parsing.get("document_intelligence"), dict) else {}
            ...
            text_rows = parsing.get("text_extraction") if isinstance(parsing.get("text_extraction"), list) else []
            parsed_count = sum(1 for row in text_rows if isinstance(row, dict) and int(row.get("text_length") or 0) > 0)
            artifact_groups = self._download_artifact_groups(downloads)
            manual_pricing_verified = str(item.get("pricing_verification_status") or "").lower() == "verified" or str(item.get("pricing_review_status") or "").upper() == "PRICING_VERIFIED"
            pricing_structure_confidence = max(
                _safe_float(intelligence.get("pricing_schedule_confidence"), 0.0),
                _safe_float(intelligence.get("boq_confidence"), 0.0),
                _safe_float(intelligence.get("extraction_confidence"), 0.0),
            )
            pricing_detected = bool(
                pricing_structure_confidence >= 0.65
                or artifact_groups["pricing_schedules"]
                or artifact_groups["boqs"]
                or enriched.get("line_items")
                or enriched.get("items")
                or manual_pricing_verified
            )
            forms_detected = bool(
                intelligence.get("has_sbd_forms")
                or intelligence.get("returnables_mentioned")
                or artifact_groups["sbd_forms"]
                or artifact_groups["buyer_forms"]
            )
            spec_detected = bool(
                parsed_count
                and (
                    intelligence.get("reference_numbers")
                    or intelligence.get("delivery_mentioned")
                    or intelligence.get("supply_mentioned")
                )
            )
            verified_line_items_present = bool(enriched.get("line_items") or enriched.get("items"))
            manual_pricing_required = bool(
                combined_paths
                and closing_ok
                and forms_detected
                and spec_detected
                and not verified_line_items_present
            )
            document_verified = bool(
                combined_paths
                and not blocker_reasons
                and document_confidence_score >= MIN_DOCUMENT_CONFIDENCE
                and (spec_detected or pricing_detected)
            )
            if document_verified:
                item["document_verification_status"] = "verified"
                item["quote_candidate_status"] = "document_verified"
            else:
                item["document_verification_status"] = item.get("document_verification_status") or "needs_review"
                item["quote_candidate_status"] = item.get("quote_candidate_status") or "needs_review"

            if manual_pricing_required:
                item["pricing_review_status"] = "REVIEW_REQUIRED_PRICING"
                item["quote_candidate_status"] = "manual_pricing_required"
                item["manual_pricing_required"] = True
                item["manual_pricing_required_reason"] = "pricing schedule requires manual completion"
                item["pricing_action"] = "prepare manual pricing schedule"
                item["pricing_review_action"] = "prepare manual pricing schedule"
                item["buyer_pack_path"] = item.get("live_buyer_pack_path", "")
                item["technical_spec_files"] = artifact_groups.get("specifications", [])
                item["returnable_files"] = artifact_groups.get("sbd_forms", []) + artifact_groups.get("buyer_forms", [])
                item["pricing_schedule_files"] = artifact_groups.get("pricing_schedules", [])

            if parsing.get("quote_safe") is False:
                blocker_reasons.append(str(parsing.get("quote_block_reason") or "compulsory_briefing_or_site_meeting_detected"))
            if not parsed_count and not intelligence:
                blocker_reasons.append("document_parse_incomplete")
            if not spec_detected:
                blocker_reasons.append("rfq_spec_document_not_detected")
            if manual_pricing_required or (not pricing_detected and not manual_pricing_verified):
                blocker_reasons.append("manual_pricing_required")
            if not forms_detected:
                blocker_reasons.append("sbd_or_returnables_manual_required")

            deduped_blockers: List[str] = []
            for reason in blocker_reasons:
                clean = str(reason or "").strip()
                if clean and clean not in deduped_blockers:
                    deduped_blockers.append(clean)
            ...
            if not blocker_reasons:
                ...
                margin = _margin_percent(priced_payload)
                if pricing.get("priced") is not True or profit < MIN_PROFIT or margin < MIN_MARGIN:
                    blocker_reasons.append(str(pricing.get("message") or "pricing_policy_failed"))
                else:
                    priced_payload["rfq_id"] = str(item.get("rfq_id") or "")
                    priced_payload["buyer_rfq_number"] = priced_payload.get("buyer_rfq_number") or priced_payload.get("rfq_number") or item.get("rfq_id")
                    priced_payload["buyer_name"] = item.get("buyer_name")
                    priced_payload["title"] = item.get("title")
                    priced_payload["estimated_profit"] = profit
                    priced_payload["estimated_margin_percent"] = margin
                    ...
                    quote_pack = self._build_quote_pack_step(priced_payload)
                    if quote_pack.get("status") != "ok":
                        blocker_reasons.append(str(quote_pack.get("message") or "quote_pack_generation_failed"))
                    else:
                        item["pricing_result"] = {k: v for k, v in pricing.items() if k != "payload"}
                        item["estimated_profit"] = profit
                        item["estimated_margin"] = margin
                        ...
                        item["quote_pack_artifacts"] = quote_pack.get("artifacts") if isinstance(quote_pack.get("artifacts"), dict) else {}
                        item["quote_number"] = quote_pack.get("quote_number", "")
                        item["quote_candidate_status"] = "quote_candidate"
                        item["document_intelligence_report_path"] = parsing.get("report_path", item.get("document_intelligence_report_path", ""))
                        item["document_parse_summary"] = {
                            "parsed_text_documents": parsed_count,
                            "pricing_schedule_detected": pricing_detected,
                            "pricing_structure_confidence": pricing_structure_confidence,
                            "boq_detected": bool(intelligence.get("has_boq") or artifact_groups["boqs"]),
            ...
            if blocker_reasons:
                ...
                quote_score = 35
                submission_score = 25
                recommended_action = "Prepare manual pricing schedule" if manual_pricing_required else "Manual review required before quote generation"
            else:
                promoted = True
                promoted_count += 1
                if item.get("current_state") not in {"PRICING_VERIFIED", "QUOTE_PACK_READY", "SUBMISSION_READY_MANUAL", "SUBMISSION_READY", "PROOF_CAPTURED", "SUBMITTED", "ARCHIVED"}:
                    self._transition(item, "QUALIFIED", "visible_validation_policy_validated", event="visible_validation")
                    self._transition(item, "DOCUMENTS_ACQUIRED", "visible_validation_documents_acquired", event="visible_validation")
                    self._transition(item, "DOCUMENTS_PARSED", "visible_validation_documents_parsed", event="visible_validation")
                    self._transition(item, "PRICED", "visible_validation_pricing_passed", event="visible_validation")
                    self._transition(item, "QUOTE_PACK_READY", "visible_validation_quote_pack_ready", event="visible_validation")
                readiness_status = "quote_ready"
                quote_score = 92
                submission_score = 68
            ...
            report_row = {
                "rfq_id": item.get("rfq_id"),
                "reference_number": enriched.get("buyer_rfq_number") or enriched.get("rfq_number") or enriched.get("reference_number") or item.get("rfq_id"),
                "title": item.get("title"),
                "buyer_name": item.get("buyer_name"),
                "source": enriched.get("source") or item.get("source"),
                "source_url": source_url,
                "detail_url": detail_url,
                "document_url": document_url,
                "closing_date": self._closing_date_value(enriched),
                "document_confidence_score": round(_safe_float(item.get("document_confidence_score") or acquisition.get("document_confidence_score") or document_confidence_score, 0.0), 4),
                "document_verification_status": item.get("document_verification_status") or ("verified" if document_verified else "needs_review"),
                "quote_candidate_status": item.get("quote_candidate_status") or ("quote_candidate" if promoted else "needs_review"),
                "pricing_review_status": item.get("pricing_review_status", ""),
                "manual_pricing_required": bool(item.get("manual_pricing_required")),
                "manual_pricing_required_reason": item.get("manual_pricing_required_reason", ""),
                "pricing_action": item.get("pricing_action", ""),
                ...
                "buyer_pack_path": item.get("buyer_pack_path", item.get("live_buyer_pack_path", "")),
                "pricing_schedule_detected": pricing_detected,
                "sbd_or_returnables_detected": forms_detected,
                "rfq_spec_document_detected": spec_detected,
                "technical_spec_files": item.get("technical_spec_files", []),
                "returnable_files": item.get("returnable_files", []),
                "quote_pack_path": item.get("quote_pack_path", ""),
                "local_quote_compilation_pack": local_pack_result.get("pack_id") if isinstance(local_pack_result, dict) else "",
                "local_quote_compilation_status": local_pack_result.get("status") if isinstance(local_pack_result, dict) else "",
            }
            report_rows.append(report_row)
            processed.append(report_row)
            ...
            display_status = (
                ...
                if manual_pricing_required
                    ...
                    "Pricing Verified"
                    if manual_pricing_verified
                    else ("Quote Ready" if promoted else "Review Required")
                )
            )
            ...
            live_updates.append(
                {
                    **live_row,
                    ...
                    "downloaded_documents": artifact_groups["documents"],
                    "documents": artifact_groups["documents"] or live_row.get("documents") or [],
                    "pricing_schedules": artifact_groups["pricing_schedules"],
                    "boqs": artifact_groups["boqs"],
                    "buyer_forms": artifact_groups["buyer_forms"],
                    "returnable_forms": artifact_groups["sbd_forms"],
                    "technical_spec_files": item.get("technical_spec_files", []),
                    "returnable_files": item.get("returnable_files", []),
                    "missing_returnables": blocker_reasons,
                    "readiness_gaps": blocker_reasons,
                    "risks": blocker_reasons,
                    "recommended_action": recommended_action,
                    "quote_readiness_score": quote_score,
                    "submission_readiness_score": submission_score,
                    "qualification_score": int(item.get("qualification_score") or (90 if promoted else 55)),
                    "validation_status": "pricing_verified" if manual_pricing_verified else ("manual_pricing_required" if manual_pricing_required else readiness_status),
                    "status": display_status,
                    "pipeline_status": "pricing_verified" if manual_pricing_verified else ("quote_ready_validated" if promoted else ("manual_pricing_required" if manual_pricing_required else "review_required")),
        ...
        if changed_items:
            self.store.update_many(changed_items, {"advanced": promoted_count, "failed": 0})
        if live_updates:
            LiveRFQStore.promote(live_updates)

        report_dir = RUNTIME_DIR / "rfq_lifecycle"
        report_dir.mkdir(parents=True, exist_ok=True)
        report_path = report_dir / "visible_rfq_validation_report.json"
        live_buyer_packs = sorted(
            {str(row.get("live_buyer_pack_path") or "") for row in report_rows if str(row.get("live_buyer_pack_path") or "").strip()}
        )
        report_payload = {
            "status": "ok",
            "generated_at": utc_now_iso(),
            "processed_count": len(processed),
            "promoted_quote_ready_count": promoted_count,
            "blocked_count": blocked_count,
            "downloaded_buyer_pack_count": len(live_buyer_packs),
            "downloaded_buyer_packs": live_buyer_packs,
            "items": report_rows,
            "files_changed": [
                str(RUNTIME_DIR / "rfq_lifecycle" / "rfqs.json"),
                str(RUNTIME_DIR / "live_rfqs.json"),
                str(report_path),
            ],
            "endpoints_used": [
                "GET /opportunities",
                "POST /rfq-lifecycle/validate-visible-opportunities",
                "POST /quote-compilation/generate-local-pack",
            ],
            "commands": [
                "curl http://127.0.0.1:8001/opportunities",
                "curl -X POST http://127.0.0.1:8001/rfq-lifecycle/validate-visible-opportunities",
            ],
            "safety": self.submission_safety_guard(),
        }
    ...
    def _queue_analytics_defaults(self) -> Dict[str, Any]:
        queue_names = list(LIFECYCLE_QUEUE_CONCURRENCY.keys())
        zero_ints = {queue_name: 0 for queue_name in queue_names}
        zero_floats = {queue_name: 0.0 for queue_name in queue_names}
        return {
            "dedicated_queues": queue_names,
            "queue_depth_by_stage": {},
            "lifecycle_queue_depth": dict(zero_ints),
            "broker_queue_depth": dict(zero_ints),
            "worker_concurrency": dict(LIFECYCLE_QUEUE_CONCURRENCY),
            "worker_utilization_by_queue": dict(zero_floats),
            "queue_wait_time": dict(zero_floats),
            "task_processing_rate": dict(zero_floats),
            "busiest_queue": "",
            "queue_bottleneck_trend": [],
            "saturation_warnings": [],
    ...
    def _normalize_queue_analytics(self, payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        normalized = self._queue_analytics_defaults()
        if not isinstance(payload, dict):
            return normalized
        ...
        for key in (
            ...
            "broker_queue_depth",
            "worker_concurrency",
            "worker_utilization_by_queue",
            "queue_wait_time",
            "task_processing_rate",
            "retry_isolation",
        ):
            if not isinstance(normalized.get(key), dict):
                normalized[key] = self._queue_analytics_defaults()[key]
        for key in ("dedicated_queues", "queue_bottleneck_trend", "saturation_warnings"):
            if not isinstance(normalized.get(key), list):
                normalized[key] = self._queue_analytics_defaults()[key]
        if not isinstance(normalized.get("busiest_queue"), str):
            normalized["busiest_queue"] = ""
        normalized["backpressure_active"] = bool(normalized.get("backpressure_active"))
        return normalized
    ...
    def _source_reliability(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in items:
            ...
            if state in {"DOCUMENTS_PARSED", "PRICED", "QUOTE_PACK_READY", "SUBMISSION_READY_MANUAL", "SUBMISSION_READY", "PROOF_CAPTURED", "SUBMITTED", "ARCHIVED"}:
    ...
    def analytics(self) -> Dict[str, Any]:
        ...
        queue_analytics = self._normalize_queue_analytics(
            self._queue_analytics(items, queue, throughput)
        )
        ...
        analytics = {
            ...
            "persistent_storage": {
                ...
                "audit_file": str(RUNTIME_DIR / "rfq_lifecycle" / "audit_events.json"),
                "analytics_file": str(RUNTIME_DIR / "rfq_lifecycle" / "analytics.json"),
            },
            "safety": self.submission_safety_guard(),
        }
    ...
    def _source_cooldown_snapshot(self) -> Dict[str, Any]:
        path = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "rfq_document_acquisition" / "source_health.json"
        rows: Dict[str, Any] = {}
        if path.exists():
            try:
    ...
    def mission_control_summary(self) -> Dict[str, Any]:
        ...
        try:
            snapshot_path = RUNTIME_DIR / "rfq_lifecycle" / "mission_control_snapshot.json"
        ...
        submission_ready_total = queue["SUBMISSION_READY"] + queue["SUBMISSION_READY_MANUAL"]
        if submission_ready_total:
            alerts.append(f"{submission_ready_total} RFQ(s) ready for controlled submission proof capture")
        if submitted == 0:
        ...
        active_good = queue["PROOF_CAPTURED"] + queue["DOCUMENTS_PARSED"] + submission_ready_total
        ...
        result = {
            ...
            "proof_capture_status": {
                "submission_ready_waiting_for_proof": submission_ready_total,
