...
def _resolve_path(value: str | Path) -> Path:
    ...
    if raw.startswith("/app/runtime/"):
        return Path.cwd() / raw.replace("/app/", "", 1)
    p = Path(value)
...
def _summarise_result(result: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    keys = [
        ...
        "service_version", "input_pdf", "blocked_reason"
    ]
...
def _continue_v48_portal(
    ...
    v474 = attach_and_smart_upload(
        autofill_plan_json=autofill_plan_json,
        cdp_url=cdp_url,
        execute_uploads=upload_allowed,
    ...
    v475 = guarded_final_submission(
        autofill_plan_json=autofill_plan_json,
        cdp_url=cdp_url,
        dry_run=not final_allowed,
        execute_final_submit=final_allowed,
