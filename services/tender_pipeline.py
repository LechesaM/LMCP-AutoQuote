...
def process_pending_submissions(limit: int = 10) -> Dict[str, Any]:
    ...
    now = _lmcp_now_iso()
    history_path = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "submission_history" / "submission_history.json"
...
_SUBMISSION_HISTORY_PATH = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "submission_history" / "submission_history.json"
