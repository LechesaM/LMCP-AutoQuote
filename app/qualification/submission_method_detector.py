...
def detect_submission_method(rfq_record_or_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if any(keyword in text for keyword in ("physical submission", "tender box", "dropbox", "drop box", "hand delivery", "courier")):
        method = "physical_delivery"
        confidence = 0.95
        target = payload.get("physical_address") or payload.get("delivery_address") or ""
    elif any(keyword in text for keyword in ("portal", "e-tender", "etender", "e submission", "electronic submission", "online submission", "e-submission")):
        method = "portal"
        confidence = 0.9
        target = url_match.group(0) if url_match else str(payload.get("portal_url") or payload.get("portal") or "")
    elif email_match or any(keyword in text for keyword in ("email submission", "submit by email", "email to", "e-mail submission")):
        method = "email"
