...
class TenderPipelineService:
        ...
        if submission_method == "email" and not locked_email:
            ...
            if isinstance(original_input, dict):
                ...
                locked_email = cls._pick_first_non_empty(
                    ...
                    original_input.get("recipient_email"),
                    original_input.get("buyer_email"),
                    expected_supplier_emails[0] if expected_supplier_emails else None,
                )
            if not locked_email:
    ...
    def _guarantee_items_exist(cls, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not candidate_items:
            candidate_items = [
                {
                    "line_number": "1",
                    "description": cls._safe_str(result.get("title") or result.get("description") or "Supply and delivery item"),
                    "unit": "Each",
                    "quantity": 1.0,
                    "unit_price": 0.0,
                    "line_total": 0.0,
    ...
    def _build_and_attach_quote_pack(cls, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            passthrough_fields = [
                ...
                "seller", "company", "items", "line_items", "buyer_pricing_schedule", "pricing_schedule_mapped",
                ...
                "skip_supplier_ingestion", "skip_email_submission", "auto_refresh_csd",
            ]
            ...
            print("[FINAL FIX] locked_email =", submission_email)

            if not submission_email:
                print("[EMAIL DEBUG 2] safe_payload._locked_submission_email =", safe_payload.get("_locked_submission_email"))
                print("[EMAIL DEBUG 2] safe_payload.recipient_email =", safe_payload.get("recipient_email"))
                print("[EMAIL DEBUG 2] safe_payload.submission_email =", safe_payload.get("submission_email"))
                print("[EMAIL DEBUG 2] safe_payload.buyer_email =", safe_payload.get("buyer_email"))
                print("[EMAIL DEBUG 2] submission_pack =", submission_pack)
                print("[EMAIL DEBUG 2] original_input_payload =", original_input_payload)
    ...
    def _run_pre_quote_supplier_flow(
        ...
        # HARD RFQ LOCK (CRITICAL ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚àö√ú PREVENT RFQ LOSS)
        rfq = (
            result.get("_locked_buyer_rfq_number")
            or result.get("buyer_rfq_number")
            or result.get("rfq_number")
        ...
        # FINAL SAFETY GUARD ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚àö√ú RFQ MUST BE LOCKED
        if payload.get("_locked_buyer_rfq_number") in [None, "", "UNKNOWN"]:
            raise ValueError("CRITICAL: RFQ not locked correctly")
        
        # ================================ 
        # FREEZE RFQ ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚àö√ú NO FURTHER CHANGES ALLOWED
        # ================================
    ...
    def _route_submission(
        ...
        # ================================
        # ‚àö√Æ¬¨¬£‚àö‚àè‚Äö√†√∂¬¨‚à´‚Äö√†√∂‚àö√ú‚Äö√†√∂‚Äö√¢‚Ä† RESTORE EMAIL LOCK AFTER STORAGE/RESTORE STEPS
        # ================================
        original_input_payload = cls._get_original_input_snapshot(payload)
    ...
    def process_single_rfq(
        ...
        try:
            ...
            # ================================
            # ‚àö√Æ¬¨¬£‚àö‚àè‚Äö√†√∂¬¨‚à´‚Äö√†√∂‚àö√ú‚Äö√†√∂‚Äö√¢‚Ä† FINAL HARD LOCK (CRITICAL)
            # ================================
            if result.get("force_quote_ready") or result.get("pipeline_test_mode"):
                result["quote_ready"] = True
                result["eligible"] = True
