...
function emptyControlledPilotDashboard() {
  return {
    rfqs_harvested: 0,
    rfqs_rejected: 0,
    rfqs_approved: 0,
...
function normalizeControlledPilotDashboard(data) {
  const pilot =
    data?.controlled_pilot_dashboard ||
    data?.pilot ||
    data?.summary?.controlled_pilot_dashboard ||
    data;

  if (!pilot || typeof pilot !== "object") {
    return {
      status: "offline",
      controlled_pilot_dashboard: emptyControlledPilotDashboard(),
    };
  }

  return {
    ...data,
    controlled_pilot_dashboard: {
      ...emptyControlledPilotDashboard(),
      ...pilot,
    },
  };
}
...
export async function getControlledPilotDashboard() {
  return firstLive(
    [
      "/dashboard/pilot",
      "/dashboard/summary",
    ...
    (data) => normalizeControlledPilotDashboard(data),
  );
}

export async function getWeeklyOperationsReport() {
  return safeJson("/dashboard/weekly-operations", {}, {
    status: "offline",
    generated_at: "",
    window_days: 7,
    summary: {
