...
export default function App() {
  ...
  const systemOn = Boolean(
    lifecycle?.safety?.system_control?.system_on ??
    lifecycle?.safety?.system_control?.effective_system_status === "on" ??
    state.policy?.policy?.enabled ??
    state.auto?.enabled
  );
  ...
  const alerts = [
    ...
    portalList.length === 0 && "Portal health endpoint not yet available",
  ].filter(Boolean);
