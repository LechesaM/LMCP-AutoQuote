...
DEFAULT_OUTPUT_DIR = _RUNTIME_DIR / "auto_submission_v46"
DEFAULT_HISTORY_PATH = _RUNTIME_DIR / "submission_history" / "v46_submission_history.json"
...
def submit_from_email_draft_payload(
    ...
    rfq = buyer_rfq_number or draft.get("buyer_rfq_number") or ""

    if not rfq:
        subject = str(draft.get("subject") or "")
        # Example: Quotation Submission - JDAMARK/DIGITALBOOK/05/2026 - LMCP-QUOTE-...
        if "Quotation Submission -" in subject and " - LMCP-QUOTE-" in subject:
            try:
                rfq = subject.split("Quotation Submission -", 1)[1].split(" - LMCP-QUOTE-", 1)[0].strip()
            except Exception:
                rfq = ""
