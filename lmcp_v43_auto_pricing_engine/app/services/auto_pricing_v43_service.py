...
SERVICE_VERSION = "V43_AUTO_PRICING_ENGINE"
...
def _looks_false_positive(text: str) -> bool:
    ...
    if any(term in t for term in FALSE_POSITIVE_TERMS):
        if not any(h in t for h in REAL_ITEM_HINTS):
            return True

    if len(t) > 420 and not any(h in t for h in REAL_ITEM_HINTS):
        return True
...
def _classify_item(text: str) -> Tuple[str, str, float]:
    ...
    if any(h in t for h in REAL_ITEM_HINTS):
...
def _extract_quantity(raw: Dict[str, Any]) -> Optional[float]:
    q = _safe_float(raw.get("quantity"))
    if q and q > 0:
        return q
    ...
    for raw in candidates:
        ...
        if _looks_false_positive(desc):
            rejected.append({
                "description": desc,
                "page": raw.get("page"),
                "reason": "false-positive policy/evaluation/contact/background text",
            })
            continue
        ...
        if category == "unknown" or not qty:
            rejected.append({
                "description": desc,
                "page": raw.get("page"),
                "reason": "not a clear supply item or quantity missing",
            })
            continue
        ...
        accepted.append({
            ...
            "reason": f"classified as {category}; quantity detected",
        })
...
def _apply_profit_floor(
    ...
    for i in items:
        ...
        estimated_profit = (new_unit_excl - i.estimated_unit_cost_excl_vat) * i.quantity
...
def get_auto_pricing_status() -> Dict[str, Any]:
    return {
        "status": "ok",
        "service_version": SERVICE_VERSION,
        "service": "V43 Auto Pricing Engine",
        "description": "Cleans V42 false positives, keeps real supply items, applies margin/profit-floor pricing, and outputs quote-engine-ready line_items.",
