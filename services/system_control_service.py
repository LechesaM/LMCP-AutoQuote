...
class SystemControlService:
    ...
    def _resolve_control_mode(self, state: Dict[str, Any]) -> str:
        """
        Keep legacy system_on semantics, but surface controlled mode when the
        V48 policy state indicates controlled operation.
        """
        candidate_sources = (
            state.get("mode"),
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if not state.get("system_on") or state.get("emergency_stop"):
