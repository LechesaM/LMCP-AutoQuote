...
export default function DashboardPage() {
  ...
  const isAdminRuntimeVisible = ["supervisor", "admin"].includes(userRole);
  ...
  return (
    <>
      ...
      {isAdminRuntimeVisible ? (
        <div className="mt-6 glass-card rounded-3xl p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-black text-white">Admin Runtime Summary</h3>
      ...
      <section className="mt-6 space-y-4">
        <div>
          <div className="text-xs font-black uppercase tracking-[.26em] text-command-cyan">Operator surface</div>
        ...
        {isAdminRuntimeVisible ? (
          <div>
            <div className="text-xs font-black uppercase tracking-[.26em] text-command-amber">Admin runtime</div>
            <div className="mt-3 grid gap-4 md:grid-cols-3 xl:grid-cols-4">
              {commandCentreRoutes
                .filter((route) =>
                  [
                ...
                .map((route) => (
                  <Link key={route.path} to={route.path} className="glass-card rounded-3xl p-5 transition hover:scale-[1.01] hover:border-command-amber/30">
                    <div className="text-xs font-black uppercase tracking-[.26em] text-command-amber">Navigate</div>
                    <div className="mt-2 text-lg font-black text-white">{route.label}</div>
                    <div className="mt-1 text-sm text-slate-400">{route.description}</div>
                  </Link>
                ))}
