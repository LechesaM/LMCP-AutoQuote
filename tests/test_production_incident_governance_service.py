...
def _write_release_certification(root: Path, *, status: str = "CERTIFIED", authority: str = "GO") -> Path:
    export_id = "20260624T090000Z-release-incident"
    ...
    payload = {
        ...
        "release_governance_history": [
            {
                "analysis_id": "production-validation:release-incident",
                "generated_at": "2026-06-24T09:00:00+00:00",
        ...
        "release_governance_history_summary": {
            "analysis_count": 1,
            "latest_analysis_id": "production-validation:release-incident",
...
def _write_rollout_bundle(
    ...
    *,
    ...
    operational_incidents: list[dict[str, object]] | None = None,
    ...
    warning_list = warnings if warnings is not None else ([] if score >= 85.0 else ["incident readiness below threshold"])
    payload = {
        ...
        "operational_incidents": operational_incidents or [],
    ...
    if freeze_active:
        payload["escalation_failures"] = [{"kind": "freeze_escalation"}]
    (bundle_dir / "production_rollout_validation.json").write_text(json.dumps(payload), encoding="utf-8")
    return bundle_dir


def test_production_incident_governance_service_ready_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.production_incident_governance_service")
    ...
    service = module.ProductionIncidentGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(validation_root, "20260624T090000Z-production-incident-ready", score=97.0)

    latest = service.latest_incident_governance()
    history = service.incident_governance_history(limit=5)

    assert latest["status"] == "ok"
    assert latest["incident_governance_authority"] == "GO"
    assert latest["audit_retention_indicators"]["retention_compliant"] is True
    assert latest["audit_completeness_indicators"]["institutional_incident_history_recorded"] is False
    assert history["count"] >= 1
    assert history["incident_governance_history"][0]["incident_governance_authority"] == "GO"


def test_production_incident_governance_service_watch_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.production_incident_governance_service")
    validation_root = tmp_path / "runtime" / "staging" / "production-rollout-validations"
    release_root = tmp_path / "runtime" / "staging" / "release-certifications"
    service = module.ProductionIncidentGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(
        validation_root,
        "20260624T090000Z-production-incident-watch",
        score=74.0,
        warnings=["incident response required"],
        freeze_active=True,
        operational_incidents=[{"kind": "supervision_failure"}],
    ...
    assert latest["incident_governance_authority"] == "WATCH"
    assert latest["audit_retention_indicators"]["retention_compliant"] is True
    assert latest["incident_governance_score"] < 85.0
