...
export async function fetchLiveSystemData() {
  const results = {
    ...
    dashboardSummary: await readJson("/dashboard/summary", {}),
    ...
    submissionHistory: await readJson("/submission_history", []),
    backendIntelligence: await readJson("/backend-intelligence/status", {}),
  };

  return {
    health: unwrap(results.health, {}),
    autonomousStatus: unwrap(results.autonomousStatus, {}),
    dashboardSummary: unwrap(results.dashboardSummary, {}),
    ...
    submissionHistory: unwrap(results.submissionHistory, []),
    backendIntelligence: unwrap(results.backendIntelligence, {}),
