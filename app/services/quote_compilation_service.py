...
def _submission_proof_mirror_path(pack_id: str) -> Path:
    return (RUNTIME_DIR / "submission_proofs" / _safe_name(pack_id, "submission-proof") / SUBMISSION_PROOF_FILENAME).resolve()
...
def _readiness_checklist_payload(
    ...
    pricing_reviewed = bool(_safe_read_pack_json(workspace, "manual_pricing.json") or _safe_read_pack_json(workspace, "pricing_schedule_completed.json"))
    quote_pack_reviewed = bool(_safe_read_pack_json(workspace, "quote_summary.json") or gate.get("pack_id"))
    buyer_pack_reviewed = bool(gate.get("can_prepare_submission") or gate.get("matched_binder") or _safe_read_pack_json(workspace, "quote_summary.json"))
    returnables_reviewed = not bool(gate.get("missing_returnables"))
    submission_method_confirmed = bool(manual_completion_present or submission_proof_present)
    manual_submission_checklist = [
        {
            "key": "buyer_pack_reviewed",
            "label": "Buyer pack reviewed",
            "passed": buyer_pack_reviewed,
            "reason": "" if buyer_pack_reviewed else "Buyer pack evidence is not yet confirmed.",
        },
        {
            "key": "pricing_reviewed",
            "label": "Pricing reviewed",
            "passed": pricing_reviewed,
            "reason": "" if pricing_reviewed else "Verified manual pricing data is not yet present.",
        },
        {
            "key": "returnables_reviewed",
            "label": "Returnables reviewed",
            "passed": returnables_reviewed,
            "reason": "" if returnables_reviewed else "Returnables are still missing or unverified.",
        },
        {
            "key": "quote_pack_reviewed",
            "label": "Quote pack reviewed",
            "passed": quote_pack_reviewed,
            "reason": "" if quote_pack_reviewed else "Quote pack has not been generated or loaded yet.",
        },
        {
            "key": "submission_method_confirmed",
            "label": "Submission method confirmed",
            "passed": submission_method_confirmed,
            "reason": "" if submission_method_confirmed else "Capture manual submission method before final proof.",
        },
        {
            "key": "final_submit_locked",
            "label": "Final submit locked/manual-only",
            "passed": True,
            "reason": "Final submission remains operator-controlled only.",
        },
    ]
    manual_submission_ready = all(bool(item.get("passed")) for item in manual_submission_checklist)
    ...
    payload = {
        ...
        "manual_submission_checklist": manual_submission_checklist,
...
def _normalize_submission_proof_paths(value: Any, field_name: str, max_count: int = 25, max_length: int = 500) -> List[str]:
    if value is None or value == "":
        return []
    if isinstance(value, str):
        values = [value]
    ...
    paths: List[str] = []
    for item in values[: max(1, int(max_count))]:
        if item is None:
            continue
        text = str(item).replace("\n", " ").replace("\r", " ").strip()
        if not text:
            continue
        if len(text) > max_length:
            raise ValueError(f"Each {field_name} entry must be {max_length} characters or fewer.")
        paths.append(text)
    return paths
...
def validate_submission_proof(payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    screenshot_paths = _normalize_submission_proof_paths(payload.get("screenshot_paths"), "screenshot_paths", max_count=25, max_length=500)
    email_proof = _safe_text(payload.get("email_proof"), 1000)
    portal_proof = _safe_text(payload.get("portal_proof"), 1000)
    receipt_reference = _safe_text(payload.get("receipt_reference"), 240)
    proof_reference = _safe_text(payload.get("proof_reference"), 240)
    ...
    if email_proof:
        proof["email_proof"] = email_proof
    if portal_proof:
        proof["portal_proof"] = portal_proof
    if receipt_reference:
        proof["receipt_reference"] = receipt_reference
    if proof_reference:
...
def _manual_pricing_path(rfq_reference: str) -> Path:
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "-", _safe_text(rfq_reference, 160)).strip("-") or "rfq"
    return (MANUAL_PRICING_DIR / f"{safe_name}.json").resolve()
...
def _artifacts_for_record(record: Dict[str, Any], artifacts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    manual_pricing_path = _manual_pricing_path(_record_reference(record))
    manual_pricing = _read_json(manual_pricing_path)
    if (
        manual_pricing_path.exists()
        and manual_pricing_path.is_file()
        and isinstance(manual_pricing, dict)
        and bool((manual_pricing.get("validation") or {}).get("verified"))
    ):
        matched.append(
            {
                "path": _relative(manual_pricing_path),
                "name": manual_pricing_path.name,
                "extension": manual_pricing_path.suffix.lower(),
                "source": "rfq_lifecycle.manual_pricing",
                "size_bytes": manual_pricing_path.stat().st_size,
                "modified_at": _modified_at(manual_pricing_path),
            }
        )
...
def _candidate_from_record(record: Dict[str, Any], artifacts: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    pricing_review_status = _safe_text(_first(record, ("pricing_review_status", "manual_pricing_status")), "")
    manual_pricing_required_value = _first(record, ("manual_pricing_required",))
    manual_pricing_required = bool(manual_pricing_required_value) if isinstance(manual_pricing_required_value, bool) else _safe_text(manual_pricing_required_value, "").lower() == "true"
    pricing_verification_status = _safe_text(_first(record, ("pricing_verification_status",)), "")
    manual_pricing_file = _manual_pricing_path(reference)
    manual_pricing = _read_json(manual_pricing_file)
    if not isinstance(manual_pricing, dict):
        manual_pricing = {}
    manual_pricing_verified = bool(
        pricing_review_status == "PRICING_VERIFIED"
        or pricing_verification_status == "verified"
        or bool(manual_pricing.get("validation", {}).get("verified"))
    )
    return {
        ...
        "pricing_verification_status": pricing_verification_status,
        "manual_pricing_required": manual_pricing_required,
        "manual_pricing_verified": manual_pricing_verified,
        "manual_pricing_path": _relative(manual_pricing_file) if manual_pricing_file.exists() else "",
...
class QuoteCompilationService:
    ...
    def save_submission_proof(
        ...
        submission_proof["buyer_rfq_number"] = _safe_text(
            submission_proof.get("buyer_reference") or submission_proof.get("buyer_rfq_number") or submission_proof.get("portal_reference") or workspace.name,
            220,
        )
        submission_proof["rfq_reference"] = _safe_text(
            submission_proof.get("buyer_rfq_number") or submission_proof.get("rfq_reference") or workspace.name,
            220,
        )
        submission_proof["quote_number"] = _safe_text(
            submission_proof.get("quote_number") or workspace.name,
            220,
        )
        submission_proof_path = _submission_proof_path(workspace)
        _write_json(submission_proof_path, submission_proof)
        proof_center_path = _submission_proof_mirror_path(workspace.name)
        _write_json(proof_center_path, submission_proof)
        try:
            from app.services.live_rfq_store import LiveRFQStore

            LiveRFQStore.upsert(
                {
                    "rfq_id": submission_proof["rfq_reference"],
                    "reference_number": submission_proof["buyer_rfq_number"],
                    "buyer_rfq_number": submission_proof["buyer_rfq_number"],
                    "quote_number": submission_proof["quote_number"],
                    "quote_pack_path": _relative(workspace),
                    "current_state": "PROOF_CAPTURED",
                    "lifecycle_state": "PROOF_CAPTURED",
                    "status": "Proof Captured",
                    "pipeline_status": "proof_captured",
                    "validation_status": "proof_captured",
                    "submission_status": submission_proof.get("submission_status") or "proof_captured",
                    "submission_checklist_status": "completed",
                    "manual_submission_ready": True,
                    "submission_ready_manual": True,
                    "proof_path": _relative(submission_proof_path),
                    "proof_file": _relative(proof_center_path),
                    "proof_manifest": _relative(proof_center_path),
                    "proof_timestamp": submission_proof.get("submission_timestamp") or submission_proof.get("saved_at") or _now_iso(),
                    "proof_indexed": True,
                    "submission_proof_path": _relative(submission_proof_path),
                }
            )
        except Exception:
            pass
        append_pack_audit_event(
            ...
            _with_operator_audit_payload({
                ...
                "screenshot_path_count": len(submission_proof.get("screenshot_paths") or []),
        ...
        return {
            ...
            "files": [
                ...
                _pack_file_entry(proof_center_path, "submission_proof_proof_center"),
            ],
            "safety": dict(MANUAL_COMPLETION_SAFETY_FLAGS),
            "read_only": True,
            "timestamp": _now_iso(),
    ...
    def generate_local_pack(self, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if candidate.get("manual_pricing_required") and not candidate.get("manual_pricing_verified"):
            return {
                "status": "pricing_required",
                "message": "Manual pricing data must be verified before local quote pack generation.",
                "candidate": candidate,
                "safety": dict(SAFETY_FLAGS),
            }
        ...
        pricing_review = {
            ...
            "manual_pricing_verified": bool(candidate.get("manual_pricing_verified")),
            "manual_pricing_path": candidate.get("manual_pricing_path", ""),
            "safety": dict(SAFETY_FLAGS),
        }
        ...
        manual_pricing = candidate.get("manual_pricing") if isinstance(candidate.get("manual_pricing"), dict) else {}
        if manual_pricing.get("line_items"):
            manual_pricing_path = workspace / "manual_pricing.json"
            _write_json(manual_pricing_path, manual_pricing)
            files.append(_file_entry(manual_pricing_path, "manual_pricing"))

            pricing_schedule_completed_path = workspace / "pricing_schedule_completed.json"
            _write_json(
                pricing_schedule_completed_path,
                {
                    "rfq_reference": candidate.get("rfq_reference"),
                    "pricing_review_status": candidate.get("pricing_review_status") or "PRICING_VERIFIED",
                    "pricing_verification_status": candidate.get("pricing_verification_status") or "verified",
                    "line_items": manual_pricing.get("line_items", []),
                    "totals": manual_pricing.get("totals", {}),
                    "validation": manual_pricing.get("validation", {}),
                    "saved_at": manual_pricing.get("saved_at") or manual_pricing.get("updated_at") or _now_iso(),
                    "safety": dict(SAFETY_FLAGS),
                },
            )
            files.append(_file_entry(pricing_schedule_completed_path, "pricing_schedule_completed"))
