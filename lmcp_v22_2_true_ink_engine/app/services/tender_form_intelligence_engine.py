...
ENGINE_VERSION = "V22.2_TRUE_INK_ENGINE"
...
def _resolve(p):
    ...
    return None
def _ink(c="black"): return (2, 2, 2, 255) if "blue" not in str(c).lower() else (3, 24, 125, 255)
def _png(img):
...
def _to_alpha(src, ink=(0,0,0,255)):
    ...
    # More aggressive extraction for light pencil/pen source.
    a=gray.point(lambda p: 255 if p < 205 else 0)
...
def _alpha_thicken(a, level=1):
    # Subtle dilation for real ink weight; avoids faint/ghost look.
...
def _norm(img,target_h=150,ink=(0,0,0,255)):
    ...
    a=img.getchannel("A")
    a=_alpha_thicken(a,1)
    a=a.point(lambda p: 0 if p < 8 else min(255, int(p*1.65)))
...
def build_glyph_library(reference_image, force=False):
    ...
    return {"status":"ok","engine_version":ENGINE_VERSION,"message":"V22.2 glyph library built with true ink amplification.","reference_image":str(ref),"glyph_dir":str(GLYPH_DIR),"expected_glyphs":len(ORDERED_GLYPHS),"saved_glyphs":len(saved),"saved_chars":"".join(saved),"rows_found":len(rows),"diagnostics":diagnostics,"checked_at":_now()}
...
def _load(ch,ink):
    ...
    a=img.getchannel("A")
    a=_alpha_thicken(a,1)
    a=a.point(lambda px: 0 if px < 6 else min(255, int(px*1.55)))
    out=Image.new("RGBA",img.size,ink); out.putalpha(a)
    return _crop(out,1)
...
def _prepare(ch,target_h,ink,seed):
    ...
    if g.height:
        sc=target_h/float(g.height)
        # Aspect ratio locked. No random scaling.
        g=g.resize((max(3,int(g.width*sc)),max(5,int(g.height*sc))),Image.LANCZOS)
    random.seed(seed)
    g=g.rotate(random.uniform(-.08,.08),expand=True,resample=Image.BICUBIC,fillcolor=(0,0,0,0))
    return _pressure(g,ink,seed)

def _measure(text,target_h,ink,seed=0):
    cs=max(.4,target_h*.030); ws=max(2.0,target_h*.22); w=0.0
...
def _text_img(text,w,h,size,ink,seed=0,align="left",max_chars=0,baseline=.76):
    ...
    target=max(6,min(int(H*.46),int(size*.96)))
    ...
    base=int(H*baseline); cs=max(.4,target*.030); ws=max(2.0,target*.22)
    for i,ch in enumerate(text):
        if ch==" ":
            x+=ws*random.uniform(.98,1.03); continue
        g=_prepare(ch,target,ink,seed+i)
        y=max(0,int(base-g.height+0.18*math.sin(i/4.0)))
        if x+g.width>W-1: break
        canvas.alpha_composite(g,(int(x),y))
        x += g.width + cs*random.uniform(.98,1.06)
    return canvas

def _xmark(w,h,ink,seed=0):
    random.seed(seed); scale=3; W,H=max(18,int(w)*scale),max(18,int(h)*scale)
    img=Image.new("RGBA",(W,H),(0,0,0,0)); d=ImageDraw.Draw(img)
    pad=int(min(W,H)*.18); lw=max(3,int(min(W,H)*.065)); j=lambda:random.uniform(-.7,.7)
...
def _training_map(payload):
    ...
    return fields, {"template":"ISIMANGALISO_CORPORATE_GIFT_PACKS","mode":"v22_2_true_ink_training_map","field_count":len(fields),"true_ink":True}
...
def complete_tender_form_intelligence(payload):
    ...
    output=OUTPUT_DIR/f"{_safe(buyer)}__v22_2_true_ink_completed.pdf"; ink=_ink(payload.get("ink_color") or "black")
    ...
    try:
        ...
        for idx,field in enumerate(fields):
            ...
            try:
                ...
                if field.mark=="x":
                    overlay=_xmark(field.w,field.h,ink,idx); typ="true_ink_x_mark"
                else:
                    overlay=_text_img(field.text,field.w,field.h,field.size,ink,seed=abs(hash((buyer,field.name,idx)))%999999,align=field.align,max_chars=field.max_chars,baseline=field.baseline); typ="true_ink_handwriting"
        ...
        return {"status":"ok","engine_version":ENGINE_VERSION,"buyer_rfq_number":buyer,"message":"Completed using V22.2 true ink engine. No typed PDF text insertion was used.","input_pdf":str(pdf),"output_pdf":str(output),"glyph_dir":str(GLYPH_DIR),"glyph_build_result":glyph_build_result,"training_map":bool(payload.get("training_map") or payload.get("auto_fill") or payload.get("use_glyph_handwriting")),"training_report":report,"inserted_count":len(inserted),"skipped_count":len(skipped),"inserted":inserted,"skipped":skipped,"checked_at":_now()}
    finally:
        doc.close()

def get_tender_form_intelligence_status():
    glyphs=list(GLYPH_DIR.glob("*.png")) if GLYPH_DIR.exists() else []
    return {"status":"ok","engine_version":ENGINE_VERSION,"mode":"true_ink_baseline_lock","typed_pdf_text_disabled":True,"single_pass_rendering":True,"ghost_suppression":True,"baseline_lock":True,"true_ink":True,"stroke_thickening":True,"glyph_count":len(glyphs),"glyph_dir":str(GLYPH_DIR),"glyph_library_exists":GLYPH_DIR.exists(),"output_dir":str(OUTPUT_DIR),"debug_dir":str(DEBUG_DIR),"checked_at":_now()}
...
def detect_sbd_pages(input_pdf):
    pdf=_resolve(input_pdf); return {"status":"ok" if pdf else "error","engine_version":ENGINE_VERSION,"input_pdf":str(pdf) if pdf else None,"message":"V22.2 uses true-ink trained map."}
def locate_writable_fields(input_pdf,page_number=1,limit=30):
    return {"status":"ok","engine_version":ENGINE_VERSION,"message":"V22.2 uses true-ink trained map; locator disabled for known template.","candidates":[]}
