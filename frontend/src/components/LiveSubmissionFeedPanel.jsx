...
export default function LiveSubmissionFeedPanel({ refreshMs = 10000 }) {
  ...
  return (
    <section className="live-feed card" id="submission-centre">
      ...
      <div className="feed-grid">
        <div className="feed-panel">
          ...
          <div className="feed-scroll proof-list-scroll">
            {proofRecords.length === 0 ? <p className="empty">No proof records found yet.</p> : proofRecords.map((record, index) => (
              <article key={`${record.record_id || record.buyer_rfq_number || record.rfq_number || "proof"}-${record.created_at || record.submitted_at || index}-${index}`} className="proof-row">
