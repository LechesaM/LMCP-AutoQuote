...
DEFAULT_OUTPUT_DIR = Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "quote_pack_v44"
DEFAULT_COMPANY_NAME = "Lechesa Manaba Consulting and Projects (Pty) Ltd"
DEFAULT_CURRENCY = "ZAR"

DEFAULT_COMPLIANCE_FILES = [
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "compliance" / "Tax_Compliance_Pin.pdf"),
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "compliance" / "CSD_Report.pdf"),
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "compliance" / "BBBEE_Certificate.pdf"),
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "compliance" / "LMCP_Company_Reg Certificate.pdf"),
    str(Path(os.getenv("LMCP_RUNTIME_DIR", "/tmp/lmcp_runtime")).expanduser().resolve() / "compliance" / "ID_LM Manaba.pdf"),
]
