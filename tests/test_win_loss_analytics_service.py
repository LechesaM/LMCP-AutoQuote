...
def test_win_loss_analytics_service_returns_governed_snapshot_and_history(tmp_path) -> None:
    module = importlib.import_module("app.services.win_loss_analytics_service")
    service = module.WinLossAnalyticsService(runtime_dir=tmp_path / "runtime" / "staging" / "historical-learning-governance")

    latest = service.latest_win_loss_analytics()
    history = service.win_loss_analytics_history(limit=5)

    assert latest["ready"] is True
    assert latest["status"] == "ok"
    assert latest["win_loss_analytics_readiness"]["ready"] is True
    assert latest["win_loss_summary"]["total_decisions"] >= 1
