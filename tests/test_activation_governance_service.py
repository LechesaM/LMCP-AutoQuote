...
def _write_release_certification(root: Path, *, status: str = "CERTIFIED", authority: str = "GO") -> Path:
    export_id = "20260623T205315Z-release-activation"
    ...
    payload = {
        ...
        "executive_rollout_summary": {
            ...
            "unresolved_deployment_blockers": [] if status == "CERTIFIED" else ["activation review required"],
            "warnings": [],
        },
        ...
        "summary_counts": {"PASS": 8, "WARN": 0 if status == "CERTIFIED" else 2, "FAIL": 0},
        "warnings": [],
    }
...
def test_activation_governance_service_ready_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.activation_governance_service")
    ...
    service = module.ActivationGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(validation_root, "20260623T205315Z-production-ready", score=96.0, status="PASS")

    latest = service.latest_activation_governance()
    history = service.activation_governance_history(limit=5)

    assert latest["status"] == "ok"
    assert latest["activation_governance_authority"] == "GO"
    assert latest["production_rollout_readiness"] is True
    assert latest["tenant_activation_readiness"]["tenant_activation_ready"] is True
    assert latest["operator_certification_readiness"]["operator_certification_ready"] is True
    assert latest["supervision_assignment_readiness"]["supervision_assignment_ready"] is True
    assert latest["throughput_expansion_readiness"]["throughput_expansion_ready"] is True
    assert latest["rollout_freeze_indicators"]["freeze_active"] is False
    assert latest["escalation_readiness"]["escalation_ready"] is True
    assert history["count"] >= 1
    assert history["activation_governance_history"][0]["activation_governance_authority"] == "GO"


def test_activation_governance_service_watch_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.activation_governance_service")
    validation_root = tmp_path / "runtime" / "staging" / "production-rollout-validations"
    release_root = tmp_path / "runtime" / "staging" / "release-certifications"
    service = module.ActivationGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(validation_root, "20260623T205315Z-production-watch", score=74.0, status="WARN", rollout_ready=True, warnings=[])

    latest = service.latest_activation_governance()

    assert latest["status"] == "watch"
    assert latest["activation_governance_authority"] == "WATCH"
    assert latest["production_rollout_readiness"] is False
    assert latest["throughput_expansion_readiness"]["throughput_expansion_ready"] is False
    assert latest["rollout_freeze_indicators"]["freeze_active"] is False
