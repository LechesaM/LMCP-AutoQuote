...
def build_glyphs(payload: Dict[str, Any]) -> Dict[str, Any]:
    return build_glyph_library(
        payload.get("reference_image") or payload.get("glyph_reference_image") or "",
...
def locate_fields(payload: Dict[str, Any]) -> Dict[str, Any]:
    return locate_writable_fields(
        payload.get("input_pdf") or payload.get("pdf_path") or "",
        int(payload.get("page") or payload.get("page_number") or 1),
