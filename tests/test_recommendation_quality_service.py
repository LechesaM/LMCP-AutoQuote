...
def test_recommendation_quality_service_returns_supervised_quality_snapshot(tmp_path) -> None:
    module = importlib.import_module("app.services.recommendation_quality_service")
    service = module.RecommendationQualityService(runtime_dir=tmp_path / "runtime" / "staging" / "recommendation-feedback-governance")

    latest = service.latest_recommendation_quality()
    history = service.recommendation_quality_history(limit=5)
