...
def test_pilot_progression_service_tracks_closure_and_expansion_candidates(tmp_path) -> None:
    module = importlib.import_module("app.services.pilot_progression_service")
    cycle_root = tmp_path / "runtime" / "staging" / "pilot-cycles"
    export_root = tmp_path / "runtime" / "staging" / "governance-exports"
    _write_cycle(cycle_root, "cycle-1", "2026-06-01T11:30:52+00:00", readiness_score=88.0)
    _write_cycle(cycle_root, "cycle-2", "2026-06-23T11:30:52+00:00", readiness_score=98.0)
    ...
    service = module.PilotProgressionService(cycle_root=cycle_root, export_root=export_root)
    latest = service.latest_progression()
    history = service.progression_history(limit=5)

    assert latest["progression_status"] == "ok"
    assert latest["progression_decision"] in {"pilot_continuation_review", "pilot_closure_review", "scope_expansion_review"}
    assert latest["pilot_progression_score"] >= 85.0
    assert latest["expansion_eligibility_indicators"]["closure_eligible"] is True
    assert history["count"] >= 1
    assert history["governance_progression_decisions"]


def test_pilot_progression_service_flags_unresolved_anomalies(tmp_path) -> None:
    module = importlib.import_module("app.services.pilot_progression_service")
    cycle_root = tmp_path / "runtime" / "staging" / "pilot-cycles"
    export_root = tmp_path / "runtime" / "staging" / "governance-exports"
    _write_cycle(
    ...
    assert latest["progression_status"] == "watch"
    assert latest["unresolved_anomaly_impact_review"] is True
    assert latest["progression_decision"] == "closure_review"
    assert latest["unresolved_blocker_summary"]["blocking_remediation_count"] >= 1
    assert latest["warning_indicators"]["unresolved_anomaly_impact_warning"] is True
