...
export default function App() {
  ...
  async function load() {
    const [health, workflow, auto, summary, pilot, weeklyOperations, opps, subSummary, profit, history, portal, radar, policy, lifecycle, lifecycleAnalytics, lifecycleTelemetry] = await Promise.all([
      getHealth(), getWorkflowHealth(), getAutonomousStatus(), getDashboardSummary(), getControlledPilotDashboard(), getWeeklyOperationsReport(), getOpportunities(), getSubmissionSummary(), getSubmissionProfit(), getSubmissionHistory(), getPortalHealth(), getRadarStatus(), getPolicy(), getRfqLifecycleMissionControl(), getRfqLifecycleAnalytics(), getRfqLifecycleTelemetry(),
    ]);
    setState({ health, workflow, auto, summary, pilot, weeklyOperations, opps, subSummary, profit, history, portal, radar, policy, lifecycle, lifecycleAnalytics, lifecycleTelemetry, loading: false, lastUpdated: new Date() });
  }
  ...
  const dashboardView = {
    ...
    pilot: state.pilot,
    weeklyOperations: state.weeklyOperations,
    setCompliancePackId,
