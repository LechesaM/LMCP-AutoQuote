...
docker compose exec -T api ls -lah "/app/runtime/playwright/downloads/" || true

echo ""
echo "6) Checking target PDF..."
TARGET_PDF="/app/runtime/playwright/downloads/RFQ_Corporate_Gift_Packs.pdf"

if docker compose exec -T api test -f "$TARGET_PDF"; then
  echo "✅ Target PDF found inside Docker:"
  docker compose exec -T api ls -lah "$TARGET_PDF"
else
