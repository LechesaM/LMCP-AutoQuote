# Supervised-Live Success Criteria

## Minimum Successful RFQs
- All 10 real valid RFQs are tracked through the supervised-live pilot set
- Every eligible RFQ completes the controlled workflow under manual governance

## Acceptable Refusal Handling
- Any refused RFQ has a documented refusal reason
- Refused RFQs do not advance to approval, review, or proof capture

## Acceptable Failure Rates
- Workflow failure rate: 0
- Persistence failure rate: 0
- Governance bypass rate: 0

## Acceptable Operator Intervention Rate
- Operator interventions are allowed only when they preserve manual governance and auditability
- Operator interventions must be recorded

## Acceptable Quote-Pack Readiness
- Quote-pack readiness remains 1.0 for eligible valid RFQs
- Quote-pack readiness failures must be investigated before release to the next stage

## Governance Compliance
- Manual approval remains mandatory
- review_ready remains mandatory before proof capture
- Proof capture remains mandatory before closeout
- Final submission remains manual-only

## Go-Forward Recommendation
- Recommend go-forward only when all governance criteria are met and no autonomous submission behavior is observed
