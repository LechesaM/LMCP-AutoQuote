...
SERVICE_VERSION = "V45_PDF_RENDERER_SUBMISSION_PREP"
DEFAULT_OUTPUT_DIR = Path("runtime/submission_pack_v45")
...
def _generate_pdf_with_reportlab(
    ...
    story.append(Paragraph("This quotation is generated from the LMCP AutoQuote pipeline and must be reviewed before final submission.", styles["Small"]))
    story.append(Paragraph("Compliance documents are bundled separately in the submission pack.", styles["Small"]))
    story.append(Paragraph(f"Generated by {SERVICE_VERSION}.", styles["Small"]))
...
def _make_email_draft(metadata: Dict[str, Any], attachments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "subject": subject,
        "body": body,
        "attachments": [a.get("path") for a in attachments if a.get("status") == "included" and a.get("path")],
...
def get_v45_status() -> Dict[str, Any]:
    return {
        "status": "ok",
        "service_version": SERVICE_VERSION,
        "service": "V45 PDF Renderer and Submission Prep",
        "description": "Renders V44 quote HTML/metadata into a formal PDF, prepares email draft JSON, copies attachments, and builds a zip-ready submission bundle.",
