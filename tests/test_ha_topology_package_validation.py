...
def test_ha_topology_package_validation_passes_with_repo_scaffold(capsys) -> None:
    module = _load_module(Path("/Users/cash/Documents/scripts/validate_ha_topology_package.py"), "validate_ha_topology_package_repo")
    report = module.build_ha_topology_package_validation_report(root_dir=Path("/Users/cash/Documents"))
    ...
    assert "k8s/base/backend-deployment.yaml" in report["manifest_inventory"]["base_files"]
    ...
    assert exit_code == 0
    assert "HA topology package validation: PASS" in output


def test_ha_topology_package_validation_detects_safety_drift_and_missing_manifests(tmp_path: Path) -> None:
    module = _load_module(Path("/Users/cash/Documents/scripts/validate_ha_topology_package.py"), "validate_ha_topology_package_bad")
    ...
    report = module.build_ha_topology_package_validation_report(root_dir=test_root)
