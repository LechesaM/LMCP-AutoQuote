...
def test_controlled_automation_orchestration_service_returns_governed_snapshot_and_history(tmp_path) -> None:
    module = importlib.import_module("app.services.controlled_automation_orchestration_service")
    service = module.ControlledAutomationOrchestrationService(runtime_dir=tmp_path / "runtime" / "staging" / "controlled-automation-governance")

    latest = service.latest_controlled_automation_orchestration()
    history = service.controlled_automation_orchestration_history(limit=5)
    ...
    assert latest["controlled_automation_orchestration_score"] >= 0
    ...
    assert latest["unresolved_automation_blockers"] is not None
