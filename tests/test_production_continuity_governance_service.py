...
def _write_release_certification(root: Path, *, status: str = "CERTIFIED", authority: str = "GO") -> Path:
    export_id = "20260624T100000Z-release-continuity"
    ...
    payload = {
        ...
        "release_governance_history": [
            {
                "analysis_id": "production-validation:release-continuity",
                "generated_at": "2026-06-24T10:00:00+00:00",
        ...
        "release_governance_history_summary": {
            "analysis_count": 1,
            "latest_analysis_id": "production-validation:release-continuity",
...
def _write_rollout_bundle(
    ...
    *,
    ...
    recovery_drill_ready: bool = True,
    ...
    warning_list = warnings if warnings is not None else ([] if score >= 85.0 else ["continuity readiness below threshold"])
    payload = {
        ...
        "recovery_drill_readiness": [{"status": "PASS" if recovery_drill_ready else "WARN"}],
        ...
        "continuity_freeze_indicators": [{"freeze_active": freeze_active}],
...
def test_production_continuity_governance_service_ready_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.production_continuity_governance_service")
    ...
    service = module.ProductionContinuityGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(validation_root, "20260624T100000Z-production-continuity-ready", score=97.0)

    latest = service.latest_continuity_governance()
    history = service.continuity_governance_history(limit=5)

    assert latest["status"] == "ok"
    assert latest["continuity_governance_authority"] == "GO"
    assert latest["audit_retention_indicators"]["retention_compliant"] is True
    assert latest["operational_continuity_scoring"]["score"] >= 85.0
    assert history["count"] >= 1
    assert history["continuity_governance_history"][0]["continuity_governance_authority"] == "GO"


def test_production_continuity_governance_service_watch_path(tmp_path: Path) -> None:
    module = importlib.import_module("app.services.production_continuity_governance_service")
    validation_root = tmp_path / "runtime" / "staging" / "production-rollout-validations"
    release_root = tmp_path / "runtime" / "staging" / "release-certifications"
    service = module.ProductionContinuityGovernanceService(validation_root=validation_root, release_certification_root=release_root)
    _write_rollout_bundle(
        validation_root,
        "20260624T100000Z-production-continuity-watch",
        score=74.0,
        warnings=["continuity review required"],
        freeze_active=True,
    ...
    assert latest["continuity_governance_authority"] == "WATCH"
    assert latest["audit_retention_indicators"]["retention_compliant"] is True
    assert latest["continuity_governance_score"] < 85.0
