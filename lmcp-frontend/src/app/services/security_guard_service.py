...
def get_operator_pin() -> str:
    return (
        os.getenv("LMCP_OPERATOR_PIN")
        or os.getenv("OPERATOR_PIN")
        or "2468"
    )


def record_security_event(event_type: str, status: str, message: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    line = {
    ...
    SECURITY_EVENTS_FILE.open("a").write(str(line) + "\n")


def verify_operator_pin(pin: str) -> Dict[str, Any]:
    expected = get_operator_pin()
    ok = str(pin or "").strip() == str(expected).strip()

    record_security_event(
        event_type="operator_pin_check",
        status="ok" if ok else "failed",
        message="Operator PIN accepted." if ok else "Operator PIN rejected.",
        metadata={"pin_provided": bool(pin)},
    )
