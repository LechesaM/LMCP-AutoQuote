...
def test_production_cutover_readiness_service_returns_governed_snapshot_and_history(tmp_path) -> None:
    module = importlib.import_module("app.services.production_cutover_readiness_service")
    service = module.ProductionCutoverReadinessService(runtime_dir=tmp_path / "runtime" / "staging" / "production-hardening-governance")

    latest = service.latest_production_cutover_readiness()
    history = service.production_cutover_readiness_history(limit=5)
