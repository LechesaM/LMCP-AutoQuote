    ...
    <style>
        ...
        * {
        ...
        .offline-shell {
            ...
            background: linear-gradient(180deg, rgba(53, 18, 28, 0.92), rgba(26, 14, 21, 0.96));
            box-shadow: var(--shadow);
        }
...
<body>
    <div class="wrapper">
        ...
        <div id="offline_shell" class="offline-shell hidden" role="status" aria-live="polite">
            <h2>Backend unavailable or offline</h2>
            <p id="offline_message">The dashboard could not reach the backend API.</p>
            <p>Expected backend URL: <code id="expected_backend_url">http://localhost:8000</code></p>
    ...
    <script>
        const EXPECTED_BACKEND_URL = window.__LMCP_API_BASE_URL__ || window.location.origin;
        ...
        function showOnlineState() {
            document.getElementById("offline_shell").classList.add("hidden");
            document.getElementById("dashboard_content").classList.remove("hidden");
        }

        function showOfflineState(errorMessage) {
            document.getElementById("offline_shell").classList.remove("hidden");
            document.getElementById("dashboard_content").classList.add("hidden");
            document.getElementById("offline_message").textContent =
                errorMessage || "The backend API could not be reached.";
            document.getElementById("expected_backend_url").textContent = EXPECTED_BACKEND_URL;
            document.getElementById("last_updated").textContent =
                "Last updated: backend unavailable or offline.";
        }
        ...
        async function loadDashboard() {
            try {
                ...
                console.error(error);
                showOfflineState(error?.message || "Backend unavailable or offline.");
            }
        }
