...
def test_operational_runbook_readiness_service_returns_governed_snapshot_and_history(tmp_path) -> None:
    module = importlib.import_module("app.services.operational_runbook_readiness_service")
    service = module.OperationalRunbookReadinessService(runtime_dir=tmp_path / "runtime" / "staging" / "production-hardening-governance")

    latest = service.latest_operational_runbook_readiness()
    history = service.operational_runbook_readiness_history(limit=5)
