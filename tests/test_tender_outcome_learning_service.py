...
def test_tender_outcome_learning_service_returns_governed_snapshot_and_history(tmp_path) -> None:
    module = importlib.import_module("app.services.tender_outcome_learning_service")
    service = module.TenderOutcomeLearningService(runtime_dir=tmp_path / "runtime" / "staging" / "historical-learning-governance")

    latest = service.latest_tender_outcome_learning()
    history = service.tender_outcome_learning_history(limit=5)
